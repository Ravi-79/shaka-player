...
shaka.text.UITextDisplayer = class {
  ...
  append(cues) {
    ...
    for (const cue of cues) {
      ...
      if (!containsCue) {
        let initialStartTime = cue.startTime;
        const initialEndTime = cue.endTime;
        const payLoadString = String(cue.payload);
        let splitStr = '';
        const inputCuePayLoadList = [];
        const isappended = payLoadString.toUpperCase() == payLoadString;
        if (isappended) {
          const splitedText = payLoadString.split('\n');
          if (splitedText.length == 1) {
            splitStr = payLoadString;
          } else {
            const firstLine = splitedText[0];
            inputCuePayLoadList.push(firstLine+'\n');
            for (let index = 1; index < splitedText.length; index++) {
              let element = splitedText[index];
              if (index != 1) {
                element = '\n' + element;
              }
              splitStr += element;
        ...
        for (let i = 0; i < splitStr.length; i += 3) {
          if (inputCuePayLoadList.length == 0) {
            inputCuePayLoadList.push(splitStr.substring(i, i + 3));
          } else {
            inputCuePayLoadList.push(
                inputCuePayLoadList[inputCuePayLoadList.length - 1] +
                  splitStr.substring(i, i + 3));
          }
        }
        for (let index = 0; index < inputCuePayLoadList.length; index++) {
          const newpayLoad = inputCuePayLoadList[index];
          let newStartTime;
          let newEndTime;
          if (index == inputCuePayLoadList.length - 1) {
            newStartTime = initialStartTime;
            newEndTime = initialEndTime;
          } else {
            newStartTime = initialStartTime;
            newEndTime = initialStartTime + 0.060;
            initialStartTime = newEndTime;
          }
          const newCue = new shaka.text.Cue(
              newStartTime, newEndTime, newpayLoad);
          this.cues_.push(newCue);
        }
      }
    }
